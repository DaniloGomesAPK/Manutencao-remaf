...
class CheckoutServiceManager {
  ...
  /**
   * Abre o WhatsApp com mensagem do Plano Mensal (R$ 30/mês)
   */
  openMonthlyCheckout(userEmail?: string): void {
    ...
    let message = 'Olá! Tenho interesse em adquirir o acesso ao sistema DG Gestão em Orçamentos no Plano Mensal (R$ 30,00/mês).';
    if (userEmail) {
  ...
  /**
   * Abre o WhatsApp com mensagem do Plano Anual (R$ 300/ano PIX)
   */
  openAnnualCheckout(userEmail?: string): void {
    ...
    let message = 'Olá! Tenho interesse em adquirir o acesso ao sistema DG Gestão em Orçamentos no Plano Anual (R$ 300,00/ano no PIX - 2 meses grátis, economia de R$ 60,00).';
    if (userEmail) {
      message += ` Meu e-mail de cadastro é: ${userEmail}.`;
    }
    const url = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`;
